...
export const ibazelListener = (cb: (state: State) => void) => {
    ...
    process.stdin.on('data', async chunk => {
        if (chunk.toString().indexOf('IBAZEL_BUILD_STARTED') !== -1) {
            ...
            startedBuildsCount++
            cb({ state: 'started' })
        } else if (chunk.toString().indexOf('IBAZEL_BUILD_COMPLETED SUCCESS') !== -1) {
            startedBuildsCount--
            clearTimeout(resumeTimeout)
            if (startedBuildsCount === 0) {
                ...
                log(`IBAZEL still building and has ${startedBuildsCount} build(s) running`)
            }
        } else if (chunk.toString().indexOf('IBAZEL_BUILD_COMPLETED FAILURE') !== -1) {
            startedBuildsCount--
            clearTimeout(resumeTimeout)
            if (startedBuildsCount === 0) {
